...
class FullyConnected(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int, int]) -> None:
        ...
        self.cache: OrderedDict = OrderedDict({"X": [], "Z": []})  # cache for backprop
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        dLdb = np.matmul(np.ones(dLdZ.shape[0]), dLdZ) #dLdZ.sum(axis = 0, keepdims = True)
...
class Conv2D(Layer):
        ...
        for row in range(out_rows):
            for col in range(out_cols):
                for oc in range(out_channels):
                    Z[:, row, col, oc] = (np.sum(X_pad[:, row * self.stride : row * self.stride + kernel_height, 
                                                       col * self.stride : col * self.stride + kernel_width, :,]
                                            * W[:, :, :, oc], axis=(1, 2, 3),)+ b[:, oc])
        out = self.activation(Z)
        self.cache["Z"] = Z
        self.cache["X"] = X
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        for row in range(out_rows):
            for col in range(out_cols):
                for oc in range(out_channels):
                    dX_pad[:, row*self.stride:row*self.stride + kernel_height, col*self.stride:col*self.stride + kernel_width, :] += W[np.newaxis, :, :, :, oc] * dZ[:, row:row+1, col:col+1, np.newaxis, oc]
                    dW[:, :, :, oc] += np.sum(X_pad[:, row*self.stride:row*self.stride + kernel_height, col*self.stride:col*self.stride + kernel_width, :] * dZ[:, row:row+1, col:col+1, np.newaxis, oc], axis=0)
        self.gradients["W"] = dW
        self.gradients["b"] = dB
...
class Pool2D(Layer):
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        for row in range(out_rows):
            for col in range(out_cols):
                row0 = row * self.stride
                row1 = row * self.stride + kernel_height
                col0 = col * self.stride
                col1 = col * self.stride + kernel_width
                if self.mode == "max":
                    X_pool[:, row, col, :] = np.max(X_pad[:, row0:row1, col0:col1, :], axis = (1, 2))
                elif self.mode == "average":
                    X_pool[:, row, col, :] = np.mean(X_pad[:, row0:row1, col0:col1, :], axis = (1, 2))
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        for row in range(out_rows):
            for col in range(out_cols):
                ...
                col1 = col * self.stride + kernel_width
                if self.mode == "max":
                    x = X_pad[:, row0:row1, col0:col1, :]
                    x = x.reshape(n_examples, kernel_height*kernel_width, in_channels)
                    indexes = np.argmax(x, axis = 1)
                    mask = np.zeros_like(x)
                    n_idx = np.indices((n_examples, in_channels))[0]
                    c_idx = np.indices((n_examples, in_channels))[1]
                    mask[n_idx, indexes, c_idx] = 1
                    mask = mask.reshape(n_examples, kernel_height, kernel_width, in_channels)
                    dldX[:, row0:row1, col0:col1, :] += mask * dLdY[:, row:row+1, col:col+1, :]
                elif self.mode == "average":
                    dldX[:, row0:row1, col0:col1, :] += dLdY[:, row:row+1, col:col+1, :] / (kernel_height*kernel_width)
